...
foreach ($startedTimers as $timer) {
    ...
    if ($timer['task_id'] != '0') {
        $data.= '<a href="'.admin_url('tasks/view/'.$timer['task_id']).'" class="_timer font-medium" onclick="init_task_modal('.$timer['task_id'].');return false;">'.get_task_subject_by_id($timer['task_id']).'</a>';
    }
...
if (total_rows('tbltaskstimers', array('task_id'=>'0', 'staff_id'=>get_staff_user_id())) == 0
