...
class Cron_model extends CRM_Model
{
    ...
    public function __construct()
    {
        if (!defined('APP_DISABLE_CRON_LOCK') || defined('APP_DISABLE_CRON_LOCK') && !APP_DISABLE_CRON_LOCK) {
            ...
            $this->lock_handle = fopen(get_temp_dir() . 'pcrm-cron-lock', 'c');
        }
        ...
        foreach ($reminders as $reminder) {
            if (date('Y-m-d H:i:s') >= $reminder['date']) {
                ...
                if ($reminder['rel_type'] == 'customer') {
                    $relTypeText = _l('client_lowercase');
                } elseif ($reminder['rel_type'] == 'lead') {
                    $relTypeText = _l('lead_lowercase');
                } elseif ($reminder['rel_type'] == 'estimate') {
                    $relTypeText = _l('estimate_lowercase');
                } elseif ($reminder['rel_type'] == 'invoice') {
                    $relTypeText = _l('invoice_lowercase');
                } elseif ($reminder['rel_type'] == 'proposal') {
                    $relTypeText = _l('proposal_lowercase');
                } elseif ($reminder['rel_type'] == 'expense') {
                    $relTypeText = _l('expense_lowercase');
                } elseif ($reminder['rel_type'] == 'credit_note') {
                    $relTypeText = _l('credit_note_lowercase');
                }
                ...
                $description = str_ireplace('{rel_link}', $rel_values['link'], $description);
                $description = str_ireplace('{rel_name}', $rel_values['name'], $description);

                $hook_data = do_action(
                    'reminder_description_cron',
                    array('description'=>$description, 'reminder'=>$reminder, 'link'=>$rel_values['link'])
                );

                $rel_values['link'] = $hook_data['link'];
                ...
                $notified = add_notification(array(
                    ...
                    'additional_data' => serialize(array(
                        $relTypeText . ' - ' . strip_tags(mb_substr($description, 0, 50)) . '...',
                    )),
                ));
                ...
                if ($reminder['notify_by_email'] == 1) {
                    $this->emails_model->send_simple_email($staff->email, _l('not_new_reminder_for', $relTypeText), check_for_links($description));
                }
