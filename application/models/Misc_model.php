...
class Misc_model extends CRM_Model
{
    ...
    public function _search_credit_notes($q, $limit = 0)
    {
        ...
        if ($has_permission_view_credit_notes || $has_permission_view_credit_notes_own) {
            ...
            $clients_fields = prefixed_table_fields_array('tblclients');
            // Invoices
            $this->db->select(implode(',', $credit_note_fields) . ',' . implode(',', $clients_fields) . ',tblcreditnotes.id as credit_note_id,CASE company WHEN "" THEN (SELECT CONCAT(firstname, " ", lastname) FROM tblcontacts WHERE userid = tblclients.userid and is_primary = 1) ELSE company END as company');
